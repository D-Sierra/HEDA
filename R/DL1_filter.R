...
DL1_filter <- function(df_luminex = df_luminex,
  ...
  if (nrow(df_luminex) > 0){ 
    forbidden_vec <- character(0)
    
    if (!is.na(allele_MM) && nzchar(trimws(allele_MM))) {
      forbidden_vec <- c(forbidden_vec, strsplit(gsub(" ", "", allele_MM), ",")[[1]])
    }
    if (!is.na(allele_compl) && nzchar(trimws(allele_compl))) {
      forbidden_vec <- c(forbidden_vec, strsplit(gsub(" ", "", allele_compl), ",")[[1]])
    }
    
    forbidden_vec <- unique(forbidden_vec[forbidden_vec != ""])
    
    df_luminex <- df_luminex %>% 
      mutate(forbidden_alleles = ifelse(rownames(df_luminex) %in% forbidden_vec, "Yes", NA)) %>% 
      select(forbidden_alleles, everything())
  }
